...
def save_batch(bids_dir, batch_dict):
    """ Save the processing pipeline in the folder derivatives\bids_pipeline

    Parameters
    ----------
    bids_dir: str, Path of the BIDS dataset
    batch_dict: dict, Dictionnary containing the information about processing pipeline
    """
    ...
    if any(batch_dict[key] for key in batch_dict):
        with open(os.path.join(batch_dir, filename), 'w+') as file:
            json_str = json.dumps(batch_dict, indent=1, separators=(
                ',', ': '), ensure_ascii=False, sort_keys=False)
            file.write(json_str)


def check_pipeline_presence_locally(pip_name, output_dir, param_var, subject_to_analyse):
    """ Check if the pipeline is present in the local directory choose by the user to save the results

    :param pip_name: str, Name of the module choose for processing
    :param output_dir: str, Path of the local output directory
    :param param_var: dict, Parameters determine by the user
    :param subject_to_analyse: dict, Subject selected by the user
    :return: Output_directory path, output_name and description datset
    """

    def determine_variant_name(name, dirlist):
        """ Determne the name of the directory according of what exits in the output directory

        :param name: str, Name of the module choose for processing
        :param dirlist: list, list of all directory with the same name
        :return: The final name of the drectory to save the process
        """
    ...
    dirlist = [file for file in os.listdir(
        output_dir) if os.path.isdir(os.path.join(output_dir, file))]
    isin = 0
    variant_list = []
    ...
    desc_data = bids.DatasetDescPipeline(
        param_vars=param_var, subject_list=subject_to_analyse)
...
class DerivativesSetting(object):
    """
    A class used to read and manage the derivatives folder in bids dataset

    Attributes
    ----------
    path: str, Path of the derivatives folder
    pipelines: list, List of the module/pipeline presents in derivatives folder
    log: str, Text about whats happening

    Methods
    -------
    DerivativesSetting(bids_dev)
        Parsing of what is inside derivatives folder
    """
    ...
    def __init__(self, bids_dev):
        ...
        :param bids_dev: str, Path of the derivatives folder
                        bids.Derivatives,  List of dictionnary coming from the parsing of bids_manager.ins_bids_class
        """
    ...
    def read_directory(self, dev_dir):
        """ Read the derivatives directory to parse and keep what is inside

        :param dev_dir: str, Path of the derivatives folder
        :return: Log to show the error if the parsing didn't go well
        """
        ...
        with os.scandir(dev_dir) as it:
            for entry in it:
                if entry.is_dir():
                    pip = bids.Pipeline(entry.name, entry.path)
                    pip.dirname = entry.path
                    empty_dir, log = self.empty_dirs(
    ...
    def is_empty(self):
        """ Check if the derivatives directory is empty or not

        :return: Boolean value determining if the directory is empty and the list of the modules present
        """

        default = ['log', 'parsing']
        pip_list = [pip['name']
                    for pip in self.pipelines if pip['name'] not in default]
        if pip_list:
            is_empty = False
        else:
            is_empty = True
    ...
    def create_pipeline_directory(self, pip_name, param_var, subject_to_analyse):
        """ Create the module directory to save the results

        :param pip_name: str, Name of the module
        :param param_var: dict, Parameters selected for process
        :param subject_to_analyse: dict, Subject selected by the user
        :return: The path, the name and the dataset_description of the module
        """

        def check_pipeline_variant(pip_list, pip_name):
            """ Find the directory with the module name

            :param pip_list: list, list of all modules present
            :param pip_name: str, Name of the module choose for processing
            :return: Boolean given the existence or not of the module, list of the variant of the modules
            """
            variant_list = []
            it_exist = False
            for pip in pip_list:
            ...
            if it_exist:
                for elt in variant_list:
                    dataset_file = os.path.join(
                        self.path, elt, bids.DatasetDescPipeline.filename)
                    if not os.path.exists(dataset_file):
                        dataset_file = None
                    desc_data = bids.DatasetDescPipeline(filename=dataset_file)
                    same_param, sub_inside, same_input = desc_data.compare_parameters(
                        param_var, subject_to_analyse)
        ...
        desc_data.write_file(jsonfilename=os.path.join(
            directory_path, bids.DatasetDescPipeline.filename))
        return directory_path, directory_name, desc_data

    def pipeline_is_present(self, pip_name):
        """ Check if the module is present in the parsing

        :param pip_name: str, Name of the module
        :return: Boolean given the presence or not, the index in the parsing of the module
        """
        is_present = False
        idx = None
    ...
    def parse_pipeline(self, directory_path, pip_name):
        """Parse the module directory to know what type of files are presents in the directory

        :param directory_path: str, Path of derivatives
        :param pip_name: str, Name of the module
        :return:
        ...
        def parse_sub_bids_dir(sub_currdir, subinfo, num_ses=None, mod_dir=None):
            """ Parse the subject directory

            :param sub_currdir: str, Path of the subject
            :param subinfo: dict, SubjectProcess instance coming from bids_manager.ins_bids_class
            :param num_ses: str, Name of the session
            :param mod_dir: str, Name of the modality
            :return:
            """
            with os.scandir(sub_currdir) as it:
                for file in it:
                    if file.name.startswith('ses-') and file.is_dir():
                        ...
                        parse_sub_bids_dir(
                            file.path, subinfo, num_ses=num_ses, mod_dir=file.name.capitalize())
                        ...
                        if ext.lower() in eval('bids.' + mod_dir + 'Process.allowed_file_formats'):
                            subinfo[mod_dir +
                                    'Process'] = eval('bids.' + mod_dir + 'Process()')
                            subinfo[mod_dir +
                                    'Process'][-1]['fileLoc'] = file.path
                            subinfo[mod_dir +
                                    'Process'][-1].get_attributes_from_filename()
                            subinfo[mod_dir +
                                    'Process'][-1]['modality'] = mod_dir
                            if subinfo[mod_dir + 'Process'][-1]['sub'] == '':
                                ...
                                if subname:
                                    subinfo[mod_dir +
                                            'Process'][-1]['sub'] = subname[0]
                                if sesname:
                                    subinfo[mod_dir +
                                            'Process'][-1]['ses'] = sesname[0]
                    elif mod_dir and file.is_dir():
                        str2add = 'Process'
                        subinfo[mod_dir +
                                str2add] = eval('bids.' + mod_dir + str2add + '()')
                        subinfo[mod_dir + str2add][-1]['fileLoc'] = file.path

        def get_attribute_filename(filename):
            """Get the attributes (sub, ses, etc) according to the name of the file and create the directory to save
    ...
    def empty_dirs(self, output_directory, rmemptysub=False):
        ...
        :param output_directory: str, Directory path to check
        :param rmemptysub: Boolean, Informing if BMP should remove the subject directory f they are empty
        :return: Boolean to say if it is empty and the log to say what has been removed
        """
        def sub_empty(subdir):
            """ Check if the subject directory is empty or not

            :param subdir: str, Path of the subject
            :return: Boolean saying if the subject directory is empty
            """
            empty_dirs = []
            emp_dirs = []
                ...
                if not dirs:
                    ...
                    for sub in dirs:
                        ...
                        if full_sub not in emp_dirs:
                            ...
                            break
                    # Add something about if it's in empty_dirs True,
        ...
        subdir = {sub: True for sub in os.listdir(output_directory) if sub.startswith(
            'sub-') and os.path.isdir(os.path.join(output_directory, sub))}
        all_files = []
                            ...
                            if pip['name'] == pip_name:
                                if 'SourceDataset' in pip['DatasetDescJSON'] and 'sub' in pip['DatasetDescJSON']['SourceDataset']:
                                    try:
                                        ...
                                        pip['DatasetDescJSON']['SourceDataset']['sub'].remove(
                                            subname)
                                        pip['DatasetDescJSON'].write_file(os.path.join(
                                            output_directory, bids.DatasetDescPipeline.filename))
                                        break
                                    except:
                                        continue
                                if 'ParticipantsProcessTSV' in pip.keys():
                                    ids = [cnt for cnt, line in enumerate(
                                        pip['ParticipantsProcessTSV']) if line[0] == sub]
                                    if ids:
                                        pip['ParticipantsProcessTSV'].pop(
                                            ids[0])
                                        pip['ParticipantsProcessTSV'].write_file(os.path.join(
                                            output_directory, bids.ParticipantsTSV.filename))
            return False, log
...
class PipelineSetting(dict):
    """
    A class used to manage the module/pipeline. Read the JSON file, run the process, and get the errors.

    Attributes
    ----------
    keylist: list, List of the elements that should be present in the dict
    soft_path: str, Path of the directory where the json file describing the modules are saved
    log_error: str, Log to save the error
    cmd_line_log: list, List of the command line used for the process
    curr_dev: DerivativesSetting, Dict with the information about what is inside the derivatievs folder
    curr_bids: bids.BidsDataset, Dict with teh information about BIDS dataset
    curr_path: str, Path with the localisation of the module
    cwdir: str, Path of the BIDS dataset
    jsonfilename: str, Name of the JSOn file describing the module
    version: str, Version of BIDS Manager-Pipeline
    bids_dir: str or BidsDataset, Bids dataset information

    Methods
    -------
    PipelineSetting(bids_dir, soft_name)
        Getting the information about the module (soft_name)
    """
    ...
    def __init__(self, bids_dir, soft_name=None, soft_path=None):
        ...
        :param bids_dir: str or BidsDataset instance, Bids dataset information
        :param soft_name: str, Name of the module
        :param soft_path: str, path of the JSON files
        """
        ...
        self.curr_dev.bp_folder = os.path.join(
            self.cwdir, 'derivatives', 'bids_pipeline')
        os.makedirs(self.curr_dev.bp_folder, exist_ok=True)

    def __setitem__(self, key, value):
        """Set the value to the key of the dict
    ...
    def copy_values(self, input_dict):
        """Copy the value inside the dict

        :param input_dict: dict, Dictionary to copy in the class
        :return: Boolean to know if the json file should be updated
        """
        rewrite = False
    ...
    def read_json_parameter_file(self, soft_name):
        """ Read the json file of the module and get the value
        ...
        self.curr_path = self['Parameters'].check_presence_of_software(
            self['Path'])
    ...
    def set_everything_for_analysis(self, results):
        """Big function to create the command line, loop the process through the subjects and run the processes.

        :param results: dict, Dictionary containing the subject and parameters selection
        :return: The log of the process and the name of the output directory
        """

        def list_for_str_format(order, idx):
            """Create the list with the inputs and outputs to write in the command line.

            :param order: dict, Giving the tag and the position in the command line
            :param idx: int, Index in order dict
            :return: List with the inputs and outputs in the right order
            """
            use_list = []
            for elt in order:
                if isinstance(elt, list):
                    if isinstance(elt[idx], list):
                        for i in range(len(elt[idx])):
                            elt[idx][i] = '"' + elt[idx][i] + '"'
        ...
        # Check the access to make sure the user can do the process
        perm, users, log_info = self.curr_bids.access.use_token_analyse(
            bids.BidsBrick.curr_user, self['Name'])
        if not perm:
            self.log_error += '{} don"t have permission to analyse data because {}.\n'.format(
                bids.BidsBrick.curr_user, log_info)
            return self.log_error, '', {}
        elif perm and ('local_output' not in results or not results['local_output']):
            perm_dev_dict = {}
            self.curr_bids.access.determine_permissions(
                self.curr_dev.path, perm_dev_dict)
            if not perm_dev_dict['permission'] == 'full':
                self.log_error += '{} don"t have permission to write in derivatives folder.\n'.format(
        ...
        try:
            ...
            self['Parameters'].update_values(
                param_vars, results['input_param'])
            ...
            # Get the attributes of the differents input and associate the subject ID
            subject_to_analyse = SubjectToAnalyse(
                results['subject_selected'], input_dict=results['input_param'])
            participants = bids.ParticipantsProcessTSV()
            # Get the output directory according to the user selection
            ...
            if not results['derivatives_output'] and not results['local_output']:
                output_directory, output_name, dataset_desc = self.curr_dev.create_pipeline_directory(
                    self['Name'], param_vars, subject_to_analyse)
            elif results['local_output']:
                # Check in the output directory if the analysis doesnot exist
                output_directory, output_name, dataset_desc = check_pipeline_presence_locally(
                    self['Name'], results['local_output'], param_vars, subject_to_analyse)
                res_outside = True
            else:
                output_name = results['derivatives_output']
                output_directory = os.path.join(
                    self.cwdir, 'derivatives', output_name)
                dataset_desc = bids.DatasetDescPipeline(
                    filename=os.path.join(output_directory, 'dataset_description.json'))
                participants.read_file(os.path.join(
                    output_directory, 'participants.tsv'))

            # Create the progression bar
            sg.theme('Light Grey 1')
            layout = [
                [sg.Text(self['Name'] + ' is running saved in ' +
                         output_name, justification='center')],
                [sg.InputText(readonly=True, key='-INPUT-')],
                [sg.ProgressBar(max_value=200, orientation='h',
                                size=(30, 20), key='progress')]
            ]
            window = sg.Window(
                'Bids Pipeline: Progression bar', layout, finalize=True)

            dataset_desc['PipelineDescription']['BIDSPipelineVersion'] = self.version
            dataset_desc.write_file(jsonfilename=os.path.join(
                output_directory, 'dataset_description.json'))

            # Get the value for the command_line
            cmd_arg, cmd_line, order, input_dict, output_dict, interm = self.create_command_to_run_analysis(
                output_directory, subject_to_analyse)
        except (EOFError, TypeError, ValueError, SystemError) as er:
            # Catch the errors and display it
            ...
            self.write_log()
            empty_dir, log_empty = self.curr_dev.empty_dirs(
                output_directory, rmemptysub=False)
            if empty_dir:
        ...
        if not order:
            # Processing for module which can manage BIDs dataset (i.e. BIDS Apps)
            window['-INPUT-'].update(
                'BIDS Pipeline cannot give the progression for this pipeline.')
            proc = subprocess.Popen(
                cmd_line, stdout=subprocess.PIPE, stderr=subprocess.PIPE, universal_newlines=True)
            ...
            # Get the input files for each subjects
            taille, idx_in, in_out, err_input = input_dict.get_input_values(
                subject_to_analyse, order)
            self.log_error += err_input
            # get the output associate to the input files
            if output_dict:
                output_dict.get_output_values(
                    in_out, taille, order, output_directory, idx_in)  # sub
            # To take into account the prefix and suffix in AnyWave and copy anywave files if necessary
            if isinstance(cmd_arg, AnyWave):
                try:
                    warn, sub_tmp_dir = cmd_arg.copy_create_anywave_files_for_analysis(
                        order, idx_in, in_out)
                    if warn:
                        self.log_error += 'Warning {}:\n'.format(
                            self['Name']) + warn
                    ...
                    self.write_log()
                    empty_dir, log_empty = self.curr_dev.empty_dirs(
                        output_directory, rmemptysub=True)
                    if empty_dir:
                        shutil.rmtree(output_directory)
                    return self.log_error, output_name, {}
            ...
            for sub in in_out:
                ...
                while idx < taille[sub]:
                    ...
                    try:
                        # there was an error here but the progression windows
                        # wasn't closed
                    ...
                    self.log_error += cmd_arg.verify_log_for_errors(
                        use_list, error_proc)
                    if error_proc[1]:
                        log_error.append(error_proc[1])
                    if not error_proc[1]:
                        self.write_json_associated(
                            use_list, output_directory, cmd_arg)
                    else:
                        log_error.append(error_proc[1])
                    idx = idx + 1
                    init = init+1
            ...
            # Delete the temporary folder used if the user doesn't have the access
            if isinstance(cmd_arg, AnyWave) and cmd_arg.move_files_refused:
                shutil.rmtree(sub_tmp_dir)
        else:
            self.log_error += datetime.datetime.now().strftime("%Y-%m-%dT%H-%M-%S") + ': ' + \
                'ERROR: The analysis {0} could not be run due to an issue with the inputs and the outputs.\n'.format(
                    self['Name'])
            self.write_log()
            window.close()
            return self.log_error, output_name, {}
        ...
        if isinstance(cmd_arg, AnyWave) and not cmd_arg.move_files_refused:
            handle_anywave_files(self.curr_bids.dirname, self.curr_bids.access,
                                 cmd_arg.curr_user, reverse=False, sublist=subject_to_analyse['sub'])
        ...
        if not empty_dir or (empty_dir and bids.BidsBrick.curr_user not in dataset_desc['Authors']):
            ...
            sub_analysed = [sub.split(
                '-')[1] for sub in os.listdir(output_directory) if sub.startswith('sub')]
            sub2remove = [sub for sub in subject_to_analyse['sub']
                          if sub not in sub_analysed]
            ...
            participants.write_file(tsv_full_filename=os.path.join(
                output_directory, bids.ParticipantsTSV.filename))
            dataset_desc.update(subject_to_analyse, sub2remove,
                                sub_analysed, order_keys=order_keys)
            dataset_desc.write_file(jsonfilename=os.path.join(
                output_directory, bids.DatasetDescPipeline.filename))
            try:
            ...
            temp = self.write_analysis_done_log(
                dataset_desc, filename=os.path.join(output_directory, Parameters.filename))
        else:
            self.log_error += datetime.datetime.now().strftime("%Y-%m-%dT%H-%M-%S") + ': ' + \
                'Warning: The folder {0} has no results so your analysis may not succeed. The folder {0} will be erased.\n'.format(
                    output_name)
            shutil.rmtree(output_directory)
        # Write the log and send it in the main windows
        self.write_log()
    ...
    def create_command_to_run_analysis(self, output_directory, subject_to_analyse):
        """Get the parameters and create the command line depending of the type of module (i.e. executable, Matlab, AnyWave)

        :param output_directory: str, Path of teh output directory
        :param subject_to_analyse: SubjectToAnalyse, Atrributes of the inputs and the subject list
        :return: Arguments of the command, the command line, the order for the different inputs and outputs, and
        intermediate name (i.e. AnyWave, Matlab)
        """
        ...
        if interm:
            cmd_arg = eval(
                interm + '(curr_path=self.curr_path, dev_path=output_directory, callname=callname, subject_to_analyse=subject_to_analyse)')
        else:
            cmd_arg = Parameters(curr_path=self.curr_path, dev_path=output_directory,
                                 callname=callname, subject_to_analyse=subject_to_analyse)

        for key in self['Parameters']:
            if isinstance(self['Parameters'][key], Arguments):
                self['Parameters'][key].command_arg(
                    key, cmd_arg, subject_to_analyse)
            elif key == 'Input':
                input_p = Input()
        ...
        cmd_line, order = cmd_arg.command_line_base(
            cmd_line_set, mode, output_directory, input_p, output_p)
        return cmd_arg, cmd_line, order, input_p, output_p, interm

    def write_json_associated(self, inout_file, output_directory, analyse):
        """Write json file associated to the results file

        :param inout_file: list, Inputs and outputs files
        :param output_directory: str, Name of the output directory
        :param analyse: dict, Containing the paramteres of the processs
        :return:
        """
        def create_json(input_file, output_json, analyse):
            """Create the json

            :param input_file: str, Name of the source
            :param output_json: str, Name of the json file
            :param analyse: dict, Dictionary with the parameters
            :return:
            """
            jsonf = bids.BidsJSON()
            ...
            jsonf['Date'] = datetime.datetime.now().strftime(
                "%Y-%m-%dT%H-%M-%S")
        ...
        if not output_fin:
            ...
            if input_file:
                ...
                potential_file = [entry for entry in os.listdir(
                    output_fin) if entry.startswith(filename)]
                is_json = False
                ...
                if not is_json and potential_file:
                    ...
                    if ext:
                        output_file = potential_file[-1].replace(
                            ext, bids.BidsJSON.extension)
                    else:
    ...
    def write_log(self):
        """Write the log file in bids_pipeline folder
    ...
    def write_command_line_log(self):
        """Write the file containing all command lines used in the process
    ...
    def write_analysis_done_log(self, dataset_desc, filename=None):
        """Write the json file saving the parameters and subject selected for the process

        :param dataset_desc: DatasetDescription, Dictionary containing all information (BIDS requirements)
        :param filename: str, Name of the file
        :return: Dictionary of the file written
        """
        ...
        if not filename:
            analysis_dir = os.path.join(
                self.curr_dev.bp_folder, 'analysis_done')
        ...
        with open(filename, 'w+') as file:
            json_str = json.dumps(file_to_write, indent=1, separators=(
                ',', ': '), ensure_ascii=False, sort_keys=False)
            file.write(json_str)
...
class Parameters(dict):
    """
    A class to manage the parameters of the module

    Attributes
    ----------
    keylist: list, List of the elements that should be present in the dict
    derivatives_directory: str, Path of the derivatives directory
    bids_directory: str, Path of the BIDS dataset
    callname: str, Name of the module to call it
    curr_path: str, Path of the Module
    curr_bids: BidsDataset, Structure of the current BIDSDataset
    mtg_file: str, Path of the montage file (specific to AnyWave)
    arg_readbids: list, List of the attributes presents in BIDS dataset
    filename: str, Name of the file to save the parameters in the module directory
    ...
    def __init__(self, curr_path=None, dev_path=None, callname=None, subject_to_analyse=None):
        ...
        :param curr_path: str, Path of the Module
        :param dev_path: str, Path of the derivatives directory
        :param callname: str, Name of the module to call it
        :param subject_to_analyse: SubjectToAnalyse, Atrributes of the inputs and the subject list
        """
        if dev_path:
    ...
    def keys_standardization(self, input_dict):
        """ Read the JSON keys and update them if they are in old version

        :param input_dict: dict, dictionary with the old version
        :return: Dictionary with the new keys value and the log for explanation
        """
        log = ''
        new_dict = {}
        if isinstance(self, InputArguments):
            ...
            all_keys = Arguments.unit_value + Arguments.read_value + Arguments.list_value + \
                Arguments.file_value + Arguments.bool_value + Arguments.bids_value
    ...
    def copy_values(self, input_dict):
        """ Copy the dictionary value in the class

        :param input_dict: dict, with the value
        :return: Boolean, to know if the JSON file needs to be overwrite
        """
        rewrite = False
        for key in input_dict:
            if key in self.keylist:
    ...
    def update_values(self, input_dict, input_param=None):
        ...
        :param input_dict: dict, with te value
        :param input_param: dict, with the attributes for the input
        :return:
        """
    ...
    def check_presence_of_software(self, curr_path):
        """Check if the path for the module mentionned in the JSON file exists
        ...
        def select_pipeline_path(name, ext):
            ...
            filename = filedialog.askopenfilename(
                title='Please select ' + name + ' file', **ask_file_options)
            if not name in filename:
                messagebox.showerror(
                    'PathError', 'The selected file is not the good one for this pipeline')
    ...
    def command_arg(self, key, cmd_dict, subject_list):
        """Associate the value to the arguments in the command line

        :param key: str, arguments
        :param cmd_dict: dict, dictionary of the arguments presents in the command line
        :param subject_list: dict, subject selection
        :return:
        """
    ...
    def command_line_base(self, cmd_line_set, mode, output_directory, input_p, output_p):
        """Create the basic of the command line (beginning)

        :param cmd_line_set: str, specific beginning (more for Docker)
        :param mode: str, automatic or Manual
        :param output_directory: str, output path
        :param input_p: list, list of dictionary of the input to get
        :param output_p: dict, dictionary of the output to get
        :return: the command line and dictionary of the order of input and output to fill later in command line
        """
        if not cmd_line_set:
        ...
        cmd_line, order = self.chaine_parameters(
            output_directory, input_p, output_p)
        cmd = cmd_base + cmd_line
        return cmd, order

    def chaine_parameters(self, output_directory, input_dict, output_dict):
        """Put the arguments and their value in command line

        :param output_directory: str, output path
        :param input_dict: list, list of dictionary of the input to get
        :param output_dict: dict, dictionary of the output to get
        :return: the command line and dictionary of the order of input and output to fill later in command line
        """
        cmd_line = ''
        cnt_tot = 0
        ...
        for clef in self:
            if clef == 'Input':
                for cn, elt in enumerate(input_dict):
                    ...
                    if elt['MultipleSubjects'] and elt['Type'] == 'dir':
                        if not elt.deriv_input:
                            ...
                            cmd_line += ' ' + elt['Tag'] + ' ' + os.path.join(
                                self.bids_directory, 'derivatives', elt['DerivFolder'][0])
                    else:
                        if elt.deriv_input:
                            if (not elt['DerivFolder'] or elt['DerivFolder'] == ['']) \
                                    and ('InRaw' in elt and not elt['InRaw']) \
                                    and elt['Optional']:
                                pass
                            elif (not elt['DerivFolder'] or elt['DerivFolder'] == ['']) \
                                    and (not elt['Optional'] or ('InRaw' in elt and not elt['InRaw'])):  # add an otion if not optional but Inraw
                                raise ValueError(
                                ...
                                cmd_line += ' ' + \
                                    elt['Tag'] + ' {' + str(cnt_tot) + '}'
                                cnt_tot += 1
                            elif 'InRaw' in elt and elt['InRaw']:
                                order[tag_val] = cnt_tot
                                cmd_line += ' ' + \
                                    elt['Tag'] + ' {' + str(cnt_tot) + '}'
                                cnt_tot += 1
                ...
                if output_dict['MultipleSubjects'] and output_dict['Directory']:
                    cmd_line += ' ' + \
                        output_dict['Tag'] + ' ' + output_directory
                else:
    ...
    def verify_log_for_errors(self, input, x):
        """Get the error from the process and write in the log

        :param input: list, input files
        :param x: value from process
        :return: log
        """
        log_error = ''
        if isinstance(x, tuple):
            if not x[0] and not x[1]:
                log_error += datetime.datetime.now().strftime("%Y-%m-%dT%H-%M-%S") + ': ' + \
                    ' '.join(input) + ' has been analyzed with no error\n'
            elif not x[1]:
                log_error += datetime.datetime.now().strftime("%Y-%m-%dT%H-%M-%S") + ': ' + \
                    ' '.join(input) + ': ' + x[0] + '\n'
            else:
                log_error += datetime.datetime.now().strftime("%Y-%m-%dT%H-%M-%S") + \
                    ': ERROR: ' + ' '.join(input) + ': ' + x[1] + '\n'

        return log_error

    def write_file(self, output_directory):
        """Write the JSON file BP_parameters

        :param output_directory: str, path of the output directory
        :return:
        """
    ...
    def get_list_subclasses_names(cls):
        """Give the list of class inherits from the class Parameters
    ...
    def _assign_bids_dir(cls, bids_dir, curr_bids):
        """Assign the BIDS directory to the class

        :param bids_dir: str, Path of the BIDS directory
        :param curr_bids: BidsDataset
        :return:
        """
...
class AnyWave(Parameters):
    """
    A subclass of Parameters, to manage AnyWave plugin. Most of the function are same than Parameters class
    """
    ...
    def __init__(self, curr_path=None, dev_path=None, callname=None, subject_to_analyse=None):
        ...
        self.anywave_folder_user = os.path.join(
            bids.BidsDataset.dirname, 'derivatives', 'anywave', self.curr_user)
        self.anywave_folder_common = os.path.join(
            bids.BidsDataset.dirname, 'derivatives', 'anywave', 'common')
        # Check the access to see if user can move files
        perm, is_used_by, info = self.curr_bids.access.use_token_analyse(
            self.curr_user, soft=self.callname)
    ...
    def chaine_parameters(self, output_directory, input_dict, output_dict):
        jsonfilename = os.path.normpath(os.path.join(
            self.derivatives_directory, self.callname + '_parameters' + '.json'))
        pref_flag = False
        suff_flag = False
        mtg_flag = False
        ...
        for cn, elt in enumerate(input_dict):
            ...
            if elt['MultipleSubjects'] and elt['Type'] == 'dir':
                ...
                if elt.deriv_input:
                    if (elt['DerivFolder'] == [''] or not elt['DerivFolder']) \
                            and (not elt['Optional'] or ('InRaw' in elt and not elt['InRaw'])):
                        raise ValueError(
                            'The derivative folder for this paramater {} should be mentionned.\n'.format(elt['Tag']))
                    elif elt['DerivFolder'] != [''] and elt['DerivFolder']:
                        order[tag_val] = cnt_tot
    ...
    def copy_create_anywave_files_for_analysis(self, order, idx_in, in_out):
        """Manage the anywave files and do a copy of them in raw data or in temporary directory if necessary.

        :param order: dict, input and output position in the command line
        :param idx_in: list, index of the input
        :param in_out: dict, the input and output files by subject
        :return: warning message and the path of the temporary directory if it exists
        """
        warning = ''
        ...
        for sub in in_out:
            for id_in in idx_in:
                ...
                if '--output_file' in order or '--output_prefix' in order:
                    pref_tag = [elt for elt in order if elt in [
                        '--output_prefix', '--output_file']][0]
                ...
                for in_idx, file in enumerate(in_out[sub][id_in]):
                    ...
                    sub_dirname = [
                        elt for elt in sub_dir[1].split(os.sep) if elt != '']
                    sub_anywave_file_user = os.path.join(
                        self.anywave_folder_user, os.sep.join(sub_dirname))
                    sub_anywave_file_common = os.path.join(
                        self.anywave_folder_common, os.sep.join(sub_dirname))
                    ...
                    if not self.move_files_refused:
                        ...
                        if '' in tmp_dir_elt:
                            ...
                            del tmp_dir_elt[idv]
                        sub_tmp_dir_write = os.path.join(
                            self.bids_directory, os.sep.join(tmp_dir_elt))
                        if isFileMeg:
                            sub_tmp_dir_write = os.path.join(
                                sub_tmp_dir_write, '_'.join(file_elt))
                    else:
                        # copy the entry
                        ...
                        if os.path.isdir(file):
                            ...
                            sub_tmp_dir_write = os.path.join(
                                sub_tmp_dir, '_'.join(file_elt))
                        else:
                            shutil.copy2(file, new_entry)
                            sub_tmp_dir_write = sub_tmp_dir
                        if isFileMeg:
                            in_out[sub][id_in][in_idx] = os.path.join(
                                new_entry, "c,rfDC")
                        else:
                        ...
                        if filename.endswith('.vhdr'):
                            ...
                            shutil.copy2(os.path.join(tmp_dir2copy, vmrkfile), os.path.join(
                                sub_tmp_dir_write, vmrkfile))
                            ...
                            shutil.copy2(os.path.join(tmp_dir2copy, eegfile), os.path.join(
                                sub_tmp_dir_write, eegfile))
                    ...
                    if get_specific_montage:
                        ...
                        file_mtg = [
                            fi for fi in listing if self.mtg_file in fi and fi.endswith('_montage.mtg')]
                        if len(file_mtg) < 1 or len(file_mtg) > 1:
                            ...
                            if self.move_files_refused:
                                filename = file_mtg[0]
                                filename_mtg = os.path.join(
                                    sub_tmp_dir_write, filename)
                                shutil.copy2(file_mtg[0], filename_mtg)
                            else:
                                filename_mtg = os.path.join(
                                    dirname, file_mtg[0])
                            in_out[sub][order['montage_file']].append(
                                filename_mtg)
                    if 'DerivFolder' not in idx_in[id_in].keys():
                        # check if it is a folder (meg)
                        if os.path.isdir(sub_anywave_file_user):
                            # rename the file for meg modality
                            filename = 'c,rfDC'
                            sub_anywave_file_user = os.path.join(
                                sub_anywave_file_user, filename)

                        for cnt, ext in enumerate(anywave_ext_analysis):
                            if os.path.exists(sub_anywave_file_user + ext):
                                shutil.copy(
                                    sub_anywave_file_user + ext, os.path.join(sub_tmp_dir_write, filename + ext))
                            elif os.path.exists(sub_anywave_file_common + ext):
                                shutil.copy(
                                    sub_anywave_file_common + ext, os.path.join(sub_tmp_dir_write, filename + ext))
                            else:
                                if cnt == 1:
                                    eventfile = os.path.join(dirname, '_'.join(
                                        file_elt[0:-1]) + '_events.tsv')
                                    mrkfile = os.path.join(
                                        dirname, filename.replace('.vhdr', '.vmrk'))
                                    if os.path.exists(eventfile):
                                        err = convert_events_in_marker_file(
                                            eventfile, modalitytype[0], sub_tmp_dir_write)
                                        if err:
                                            warning = warning + err
                                    elif os.path.exists(mrkfile):
                                        err = convert_events_in_marker_file(
                                            mrkfile, modalitytype[0], sub_tmp_dir_write)
                                        if err:
                                            warning = warning + err
                                elif cnt == 2 and not get_specific_montage:
                                    channelfile = os.path.join(dirname, '_'.join(
                                        file_elt[0:-1]) + '_channels.tsv')
                                    if os.path.exists(channelfile):
                                        err = convert_channels_in_montage_file(
                                            channelfile, modalitytype[0], sub_tmp_dir_write)
                                        if err:
                                            warning = warning + err
        if not self.move_files_refused:
            sub_tmp_dir = None
...
class Docker(Parameters):
    """
        A subclass of Parameters, to manage BIDS Apps.
    """
    keylist = ['command_line_base', 'Intermediate', 'Callname']
    ...
    def command_line_base(self, cmd_line_set, mode, output_directory, input_p, output_p):
        ...
        cmd_base = cmd_line_set + ' -v ' + self.bids_directory + ':/bids_dataset:ro -v ' + \
            self.derivatives_directory + ':/outputs ' + \
            self.callname + ' /bids_dataset /outputs'
...
class Matlab(Parameters):
    """
        A subclass of Parameters, to manage Matlab script.
    """
    ...
    def command_line_base(self, cmd_line_set, mode, output_directory, input_p, output_p):
        ...
        if mode == 'automatic':
            if not cmd_line_set:
                cmd_line_set = base + \
                    " -nosplash -nodesktop -r \"cd('" + \
                    os.path.dirname(self.curr_path) + "'); "
            cmd_end = '; exit\"'
        elif mode == 'manual':
            if not cmd_line_set:
                cmd_line_set = base + \
...
class Python(Parameters):
    """
        A subclass of Parameters, to manage Python script.
    """
...
class ParametersSide(list):
    """
        A class to manage some Parameters inside Parameters class
    """
...
class Mode(ParametersSide):
    """
        A subclass of ParametersSide, to manage the mode.
    """
...
class Input(ParametersSide):
    """
        A subclass of ParametersSide, to manage the inputs.
    """
    ...
    def control_length_of_multiple_input(self):
        """ Check if the number of files of the differents input are similar
    ...
    def get_input_values(self, subject_to_analyse, order):
        """Collect the different input files according to the subject

        :param subject_to_analyse: SubjectToAnalyse, subject selection
        :param order: dict, input position in command line
        :return: Subjects that have been remove from the analysis, index of the input and output, log with the errors to display
        """
        def compare_and_reorder(in_out, idx_in):
            """ Compare the different inputs and combine or/and reorder them to match

            :param in_out: list, List of inputs and output files
            :param idx_in: list, index of the input in in_out
            :return: log of error
            """
            err = ''
        ...
        def reorder_inputs(in_out, idx_in, key2order=None):
            """Sort the input files to match (i.e. task-rest with task-rest, etc)

            :param in_out: list, List of inputs and output files
            :param idx_in: list, index of the input in in_out
            :param key2order: list, input to sort
            :return:
            """
            ...
            if len(key2order) > 1:
                if all(idx_in[elt]['Type'] == 'file' for elt in idx_in if elt in key2order):
                    ...
                    for cnt, file in enumerate(in_out[idx_list[0]]):
                        ...
                        val_ind = {
                            elt: val for elt in SubjectToAnalyse.keylist for val in file_split if val.startswith(elt+'-')}
                        for id in idx_list[1:]:
                            val_diff = [val_ind[key] for key in val_ind if not all(
                                val_ind[key] in subfile for subfile in in_out[id])]
                            if val_diff:
                                new_file = [flist for flist in in_out[id] if all(
                                    val in flist for val in val_diff)]
        ...
        for cn, elt in enumerate(self):
            ...
            if tag_val not in order_tag:
                ...
                idx = order[tag_val]
            if not not_inside:
                input_att['Input_' + tag_val] = elt.get_input_values(
                    subject_to_analyse, in_out, idx, order_tag[idx])
                idx_in[idx] = {'Type': elt['Type'],
                               'CombinationMode': elt['CombinationMode']}
        sub_to_delete = []
        for sub in in_out:
            if len(idx_in) > 1:
            ...
            for sub in input_att[inp]:
                if sub not in sub_to_delete:
                    for key, val in input_att[inp][sub].items():
                        ...
                        subject_to_analyse[inp][key] = list(
                            set(subject_to_analyse[inp][key]))
...
class InputArguments(Parameters):
    """
        A subclass of Parameters, to manage the Inputs.
    """
    ...
    def copy_values(self, input_dict):  # , flag_process=False):
        ...
        if self['MultipleSubjects'] and self['Type'] == 'file':
            raise ValueError(
                'Your json is not conform.\n It is not possible to have files as input and process multiple subject.\n')

        return rewrite

    def update_values(self, input_dict):
        for key in input_dict:
            if key in self.keys() and key in ['DerivFolder', 'FileType', 'InRaw']:
                self[key] = input_dict[key]
        # self['ValueSelected] = input_dict

    # sub, input_param=None):
    def get_input_values(self, subject_to_analyse, in_out, idx, tag):
        """Get the input files or directory according to the input attributes

        :param subject_to_analyse: SubjectToAnalyse, Atrributes of the inputs and the subject list
        :param in_out: dict, the input and output files by subject
        :param idx: int, index of the specific input arguments
        :param tag: str, Tag (name) of the specific input arguments
        :return: dictionary of attributes used for the selection
        """
        def check_dir_existence(bids_directory, chemin, filetype=None):
            chemin_final = os.path.join(
                self.bids_directory, os.sep.join(chemin))
        ...
        if 'InRaw' in self and self['InRaw']:
            inraw = True
        else:
            inraw = False
        if self.deriv_input and not inraw:
            if self['DerivFolder'] and self['DerivFolder'] != ['']:
                ...
                raise ValueError(
                    'You have to select a derivatives folder for the input {}'.format(tag))
        if self['Type'] in ['file', '4D']:
            for sub in subject_to_analyse['sub']:
                in_out[sub][idx], input_att[sub] = self.get_subject_files(
                    subject_to_analyse['Input_'+tag], sub)
        elif self['Type'] == 'dir':
            ...
            for sub in subject_to_analyse['sub']:
                ...
                filetype = None
                if self.deriv_input:
                    chemin.append('derivatives' + os.sep +
                                  self['DerivFolder'][0])
                ...
                for key, val in path_key.items():
                    if val and key != 'modality':
                        ...
                        if os.path.exists(subdir):
                            ses = [f for f in os.listdir(subdir) if os.path.isdir(
                                os.path.join(subdir, f)) and f.startswith('ses-')]
                            if len(ses) == 1:
                                chemin.append(ses[0])
                    # elif not val:
                    #     break
                ...
                in_out[sub][idx] = [check_dir_existence(
                    self.bids_directory, chemin, filetype=filetype)]
        elif self['Type'] == 'montage':
            for sub in subject_to_analyse['sub']:
                in_out[sub][idx] = self.get_montage_files(
                    subject_to_analyse['Input_'+tag], sub)
        return input_att

    def get_subject_files(self, subject, sub_id):
        """Get the files depending on the attributes

        :param subject: SubjectToAnalyse, Atrributes of the inputs and the subject list
        :param sub_id: str, subject ID in BIDS
        :return: List of input files and dictionary of attributes used for the selection
        """
        input_files = []
        temp_att = {}
            ...
            for sub in self.curr_bids.curr_pipeline['Pipeline']['SubjectProcess']:
                if sub['sub'] == sub_id:
                    for mod in subject['modality']:
                        if not mod:
                            raise EOFError('Modality is missing.\n')
                        if not mod.endswith('Process'):
                            mod = mod + 'Process'
                        if sub[mod]:
                            for elt in sub[mod]:
                                attrdict = elt.get_attributes(
                                    ['modality', 'fileLoc'])
                                is_equal = [True]
                                for key in subject:
                                ...
                                if (all(is_equal) and elt['fileLoc'].endswith(self['FileType'])) or (all(attrdict[k] == '' for k in attrdict) and elt['fileLoc'].endswith(self['FileType'])):
                                    key_attributes = [
                                        clef for clef in SubjectToAnalyse.keylist if not clef == 'modality']
                                    ...
                                    if self['FileType'] == "" and mod != 'MegProcess':
                                        # to save the folder and not a specific file
                                        filename = os.path.join(
                                            self.bids_directory, elt['fileLoc'])
                                        filename = filename.split('\\')
                                        filename = '\\'.join(filename[:-1])
                                        # prevent adding multiple times the same folder
                                ...
                                if (all(is_equal) and mod == 'MegProcess' and self['FileType'] != ""):
                                    key_attributes = [
                                        clef for clef in SubjectToAnalyse.keylist if not clef == 'modality']
                                    for key in key_attributes:
                                        if key in elt and elt[key] and key not in temp_att:
                                            ...
                                            temp_att[key].append(elt[key])
                                    filename = os.path.join(
                                        self.bids_directory, elt['fileLoc'], 'c,rfDC' + self['FileType'])
                                    if os.path.exists(filename):
                                        input_files.append(filename)

        elif self.deriv_input and self['DerivFolder'][0].startswith('freesurfer'):
            dir_free, fle_type = self['FileType'].split('/*')
            sub_path = os.path.join(self.curr_bids.dirname, 'derivatives',
                                    self['DerivFolder'][0], 'sub-' + sub_id, dir_free)
                ...
                if sub['sub'] == sub_id:
                    for mod in subject['modality']:
                        for elt in sub[mod]:
                            ...
                            if all(is_equal):
                                key_attributes = [
                                    k for k in SubjectToAnalyse.keylist if not k == 'modality']
                                for key in key_attributes:
                                    if key in elt and elt[key] and key not in temp_att:
                                        temp_att[key] = [elt[key]]
                                    elif key in elt and elt[key] and elt[key] not in temp_att[key]:
                                ...
                                if mod == 'Meg' and os.path.isdir(input_files[-1]) and self['Type'] == 'file':
                                    input_files[-1] = os.path.join(
                                        input_files[-1], 'c,rfDC')
        # subject.update(temp_att)
        return input_files, temp_att

    def get_montage_files(self, subject, sub_id):
        """Get the input files of type montage

        :param subject: SubjectToAnalyse, Attributes of the inputs and the subject list
        :param sub_id: str, subject ID in BIDS
        ...
        if 'modality' not in subject.keys():
            subject['modality'] = self['Modality']
        if 'FileType' in self and isinstance(self['FileType'], str):
            filetype = [self['FileType']]
        elif 'FileType' in self and isinstance(self['FileType'], list):
            filetype = self['FileType']
        if self['InRaw']:
            maindir = self.curr_bids.dirname
        else:
            maindir = self.curr_bids.curr_pipeline['Pipeline'].dirname
            is_anywave = False
            if self['DerivFolder'][0] not in maindir:
                for pipeline in self.curr_bids["Derivatives"][-1]["Pipeline"]:
                    if pipeline["name"] == self['DerivFolder'][0]:
                        # when is empty, most of the time is a anywave subfolder :$
                        if not pipeline["ParticipantsProcessTSV"]:
                            is_anywave = True

                if is_anywave:
                    maindir = os.path.join(
                        self.curr_bids.dirname, 'derivatives', 'anywave', self['DerivFolder'][0])
                else:
                    maindir = os.path.join(
                        self.curr_bids.dirname, 'derivatives', self['DerivFolder'][0])
                # if not os.path.exists(maindir):
                #     maindir = os.path.join(
                #         self.curr_bids.dirname, 'derivatives', 'anywave', self['DerivFolder'][0])
            if not os.path.exists(maindir):
                raise FileExistsError(
                    'The derivatives folder {} doesn"t exist.\n'.format(maindir))

        # checking if sub exists... it should
        raw_sub = [sub for sub in self.curr_bids['Subject']
                   if sub['sub'] == sub_id][0]
        if raw_sub:
            for mod in subject['modality']:
                if 'ses' in subject:
                    ...
                    session = [elt['ses'] for elt in raw_sub[mod]]
                    session = set(session)
                for ses in session:
                    sesname = ''
                    if ses:
                        sesname = 'ses-'+ses
                    path = os.path.join(
                        maindir, 'sub-'+raw_sub['sub'], sesname, mod.lower())
                    listing = os.listdir(path)
                    possible_req = ['run', 'task', 'acq', 'proc']
                    req = [(k, v)
                           for k, v in subject.items() if k in possible_req]

                    for file in listing:
                        if not file.endswith('.mtg'):
                            continue

                        has_all = [False]*len(req)
                        for i, pair in enumerate(req):
                            for v in pair[1]:
                                if v == '':
                                    has_all[i] = True
                                elif (pair[0]+'-'+v) in file:
                                    has_all[i] = True

                        if all(has_all):
                            input_files.append(os.path.join(path, file))

                        # file = [os.path.join(path, file) for file in listing if file.endswith('.mtg') and any(
                        #         ft in file for ft in filetype) and os.path.isfile(os.path.join(path, file))]
...
class Output(Parameters):
    ...
    def copy_values(self, input_dict, flag_process=False):
        ...
        if (input_dict['Type'] == 'dir' and not input_dict['Directory']) or (input_dict['Type'] == 'file' and input_dict['Directory']):
            raise KeyError('The output in your json is not conform due to the type.\n You mention directory: {0} and type: {1}.\n'.format(
                input_dict['Directory'], input_dict['Type']))
        for key in keys:
            self[key] = input_dict[key]
        if self['MultipleSubjects'] and self['Type'] == 'file':
            raise ValueError(
    ...
    def get_output_values(self, in_out, taille, order, output_directory, idx_in):
        """ Get the output files or directory according to the input attributes

        :param in_out: dict, the input and output files by subject
        :param taille: int, number of inputs
        :param order: dict, input and output position in the command line
        :param output_directory: str, Path of the output directory
        :param idx_in: list, index of the input
        :return:
        ...
        def create_output_file(output_dir, filename, extension, bids_directory):
            """ Create the output filename depending on the input file

            :param output_dir: str, path of the output directory
            :param filename: str, name of the input file
            :param extension: list or str, extension of the output file
            :param bids_directory: str, path of the bids directory
        ...
        def create_output_sub_dir(output_dir, filename, bids_directory):
            """ Create the subject output directory to keep the BIDS format in derivatives
                ...
                if not taille:
                    ...
                    for sub in in_out:
                        ...
                        for filename in in_out[sub][idi]:
                            out_file = create_output_file(
                                output_directory, filename, self['Extension'], self.bids_directory)
                            output_files.append(out_file)
                        in_out[sub][idx] = output_files
            # elif self['Type] == 'sub':
            #     if not self['MultipleSubjects]:
            #         output_files = sub
        # A revoir avec subject_list
        else:
            if self['MultipleSubjects']:
                for sub in in_out:
                    # [output_directory] * taille[sub]
                    in_out[sub][idx] = output_directory
            else:
                for sub in in_out:
                    output_files = []
                    idx_list = list(idx_in.keys())
                    if len(idx_list) > 1:
                        ...
                        idi = idx_list[0]
                    for filename in in_out[sub][idi]:
                        output_dir = create_output_sub_dir(
                            output_directory, filename, self.bids_directory)
                        output_files.append(output_dir)
                    in_out[sub][idx] = output_files

    def command_arg(self, output_dir, soft_name, subject_list, input_type, input_file_list=None):
        """ Get the value of the Output argument

        :param output_dir: str, path of the output directory
        :param soft_name: str, name of the module
        :param subject_list: SubjectToAnalyse, Atrributes of the inputs and the subject list
        :param input_type: str, type of input (i.e. file, sub, etc)
        :param input_file_list: list, list of input files
        :return: Output with specific value for each attributes
        """
        ...
        def create_output_folder(output_dir, bids_directory, input_name=None, subject_to_analyse=None):
            """ Create the output directory to keep the BIDS format in derivatives

            :param output_dir:
            :param bids_directory:
            :param input_name:
            :param subject_to_analyse:
            :return:
            """
            out_file = []
            if input_name:
        ...
        if not self['Directory']:
            if self['Type'] == 'file':
                ...
                for filename in input_file_list:
                    out_file = create_output_file(
                        output_dir, filename, self['Extension'], soft_name, self.bids_directory)
            ...
            if self.multiplesubject:
                ...
                output_file_list = create_output_folder(
                    output_dir, self.bids_directory, input_name=input_file_list)
            elif input_type == 'sub':
                output_file_list = create_output_folder(
                    output_dir, self.bids_directory, subject_to_analyse=subject_list)
...
class Arguments(Parameters):
    """
        A subclass of Parameters, to manage the different Arguments i.e. type of parameters of the module.
    """
...
class SubjectToAnalyse(Parameters):
    """
        A subclass of Parameters, to manage the Subject selected for analysis
    """
    # required_keys = 'sub'
    ...
    def write_file(self, output_directory):
        filename = os.path.join(
            output_directory, 'subjects_selected_file.json')
        with open(filename, 'w+') as f:
            json_str = json.dumps(self, indent=1, separators=(
                ',', ': '), ensure_ascii=False, sort_keys=False)
            f.write(json_str)
...
def verify_subject_has_parameters(curr_bids, sub_id, input_vars, param=None):
    """ Function to check if the subject selected have the required file for the analysis
    ...
    for key in input_vars:
        ...
        if 'DerivFolder' in input_vars[key].keys():
            ...
            # or deriv_folder == ['']:
            if deriv_folder == '' or 'Previous analysis results' in deriv_folder:
                continue
        keylist = [clef for clef in input_vars[key] if clef not in [
            'modality', 'DerivFolder', 'FileType', 'InRaw']]
        ...
        for clef in keylist:
            for sid in sub_id:
                if deriv_folder is not None:
                    ...
                    curr_bids.curr_pipeline['Pipeline'].is_subject_present(
                        sid, True)
                    if curr_bids.curr_pipeline['Pipeline'].curr_subject['isPresent']:
                        if not mod.endswith('Process'):
                            mod = mod+'Process'
                        elmt = [
                            val[clef] for val in curr_bids.curr_pipeline['Pipeline'].curr_subject['SubjectProcess'][mod]]
                        if clef == 'mod':
                    ...
                    if curr_bids.curr_subject['isPresent']:
                        elmt = [val[clef]
                                for val in curr_bids.curr_subject['Subject'][mod]]
                        if clef == 'mod':
                            elmt = [val['modality']
                                    for val in curr_bids.curr_subject['Subject'][mod]]
                    else:
...
def main(argv):
    ...
    try:
        opts, args = getopt.getopt(argv, "hb:a:p:s:", [
                                   "bidsdirectory=", "analysisname=", "parameters=", "subjectlist="])
    ...
    for opt, arg in opts:
        if opt == '-h':
            print(
                'pipeline_class.py -b <bidsdirectory> -a <analysisname> -p <parameters> -s <subjectlist>')
            sys.exit()
            ...
            if not isinstance(subjectlist, dict):
                print(
                    'ERROR: Subject should be a dictionnary with at least sub as key.\n')
                sys.exit(-1)
...
if __name__ == '__main__':
    ...
    log_analyse = soft_analyse.set_everything_for_analysis(
        parameters, subjectlist)
    print(log_analyse)
